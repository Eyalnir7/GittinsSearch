...
DecisionRule place {
  A, X, Z,
  { (gripper A) (object X) (tableS Z) (stable A X) (busy A) }
  { (stable A X)! (stableOn Z X) (on X Z) komo(above X Z) komo(touch X Z) (busy A)! }
}

#DecisionRule place {
#  X, Y, Z,
#  { (grasped X Y) (tableS Z) (held Y) }
#  { (placed Y Z) (on Y Z) (grasped X Y)! komo(touch X Y) (busy X)! (busy Y)!
#    (notheld Y) (INFEASIBLE grasp ANY Y)! block(INFEASIBLE grasp ANY Y)
#    komo(above Y Z) (stableOn Z Y) (stable ANY Y)! } #komo(liftDownUp X) #komo(place X Y Z)=1. }
#}
