...
moves, touch, stable, dynamicOn, impulse, goForward
...
DecisionRule genStable{ A, B
    ...
    { (stable ANY B)! (dynamicOn ANY B)! (stable A B) (goable) }
}

DecisionRule genDynamicOn{ A, B
    { (dynamicOn A B)! (table A) (object B)}
    { (stable ANY B)! (dynamicOn ANY B)! (dynamicOn A B) (goable) }
}
