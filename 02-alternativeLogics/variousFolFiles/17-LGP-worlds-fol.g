...
DecisionRule grasp {
  ...
  { (grasped X Y) (held Y) (animate Y) (busy X) komo(flagClear Y) komo(touch X Y) komo(stable X Y)  } #komo(grasp X Y) } #komo(lift X)
}
...
DecisionRule handover {
  ...
  { (grasped X Y)! (grasped Z Y) (busy X)! (busy Z) komo(flagClear Y) komo(touch Z Y) komo(stable Z Y) } #komo(handover X Y Z) }
}
...
DecisionRule place {
  ...
  { (placed Y Z) (on Y Z) (grasped X Y)! (busy X)! (busy Y)! (notheld Y) (INFEASIBLE grasp ANY Y)! block(INFEASIBLE grasp ANY Y)  komo(flagClear Y) komo(above Y Z) komo(stableOn Z Y) } #komo(place X Y Z)=1. }
}
...
DecisionRule place2 {
  ...
  { (placed OBJ TAB) (busy OBJ)! (on OBJ ANY)! (on OBJ TAB) (INFEASIBLE grasp ANY OBJ)! block(INFEASIBLE grasp ANY OBJ) komo(flagClear OBJ) komo(above OBJ TAB) komo(stableOn TAB OBJ) } #komo(place OBJ TAB)=1. }
}
...
DecisionRule throw {
  ...
  { (notheld OBJ) (grasped X OBJ)! (busy X)! komo(dynamic OBJ) } # komoThrow(OBJ)=1. }
}
...
DecisionRule hit {
  ...
  { (animate OBJ) (on OBJ ANY)! komo(touch BAT OBJ) komo(impulse BAT OBJ) komo(dynamic OBJ) } # komoHit(BAT, OBJ) }
}
...
DecisionRule hitSlide {
  ...
  { (animate OBJ) komo(touch BAT OBJ) komo(impulse BAT OBJ) komo(dynamicOn TABLE OBJ) komo(fricSlide BAT TABLE)! } # komoHit(BAT OBJ TABLE) }
}
