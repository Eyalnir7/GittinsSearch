...
DecisionRule grasp {
  ...
  { (stableOn ANY Y)! (above Y ANY)! (on ANY Y)!
    (grasped X Y) (held Y) (busy X) (touch X Y) (stable X Y)
    } #komo(liftDownUp X) #komo(grasp X Y) } #komo(lift X)
}
...
DecisionRule place {
  ...
  { (grasped X Y)! (busy X)! (busy Y)! (held Y)! (stable ANY Y)! (touch X Y)!
    (on Y Z) (above Y Z) (stableOn Z Y) tmp(touch X Y)
